...
describe("SEG IO + Mesh + Video", () => {
  it("SEG import/export works with preserved references", () => {
    ...
    expect(() => applyBrush(new Uint8Array(), 1, 1, { x: 0, y: 0, r: 1 })).toThrowError(/NotImplemented/);
  });

  it("Mesh exports STL and GLB", async () => {
    // @req: F-011
    expect(() => meshFromLabelmap(new Uint8Array(), [1, 1, 1], [1, 1, 1])).toThrowError(/NotImplemented/);
  });
  ...
  it("Video export produces H.264 MKV by default", async () => {
    ...
    } as any)).rejects.toThrowError(/NotImplemented/);
  });
});
